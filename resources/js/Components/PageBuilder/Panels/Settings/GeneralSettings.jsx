...
const GeneralSettings = ({ widget, onUpdate, onWidgetUpdate }) => {
  ...
  const updateGeneral = (property, value) => {
    // Ensure general is object, not array (PHP json_decode quirk)
    const currentGeneral = Array.isArray(localWidget.general) ? {} : (localWidget.general || {});
    const updatedWidget = {
      ...localWidget,
      general: {
        ...currentGeneral,
  ...
  const updateGeneralPath = (path, value) => {
    ...
    // Ensure general is a plain object, not an array
    // PHP json_decode('{}', true) returns [] which JS treats as Array
    // String-keyed properties on arrays are lost during JSON.stringify
    if (!updatedWidget.general || Array.isArray(updatedWidget.general)) {
  ...
  const getPageId = () => {
    // First: Try currentPageId from store
    const { currentPageId } = usePageBuilderStore.getState();
    if (currentPageId) {
      return currentPageId;
    }

    // Second: Try to get page ID from URL path (e.g., /admin/page-builder/edit/4)
    const match = window.location.pathname.match(/\/page-builder\/edit\/(\d+)/);
    if (match && match[1]) {
      return parseInt(match[1]);
    }

    // Third: Fallback to window or widget context
  ...
  const handleGlobalSave = async () => {
    ...
    try {
      ...
      // Ensure arrays are converted to objects (PHP json_decode quirk)
      const ensureObj = (v) => (!v || Array.isArray(v)) ? {} : v;
      const allSettings = {
        widget_type: widgetType,
        general: ensureObj(localWidget.general) || ensureObj(localWidget.content) || {},
        style: ensureObj(localWidget.style) || {},
        advanced: ensureObj(localWidget.advanced) || {}
      };
